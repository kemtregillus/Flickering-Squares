...
Screen('DrawText',window,'Move the mouse to adjust the frequency. Click when satisfied with your choise.',xCenter/4,yCenter,[1 1 1])
...
for k = 1:nTrials
    ...
    if positionPick == 1 %% adapt field on the left
        ...
        xRSquares = xCenter+125;  %%location response squares
        yRSquares = yCenter;
        AdaptField(1,k) = 1;
    else %% adapt field on the right
        xRSquares = xCenter-125;  %%location stim squares - overall width of square matix is 375
        yRSquares = yCenter;
...
%             rLumVal = 0.5*sin(respFreq*time*(2*pi)+(rLumValnew))+0.5;
    ...
    %% save stuff
    means = cat(1,mean.',meanFreqMat,singleFreqMat);
    ...
    saveFile = strcat(subj,'_ens_1freq_',date,'.mat');
